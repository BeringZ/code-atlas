// Code Atlas 2.0 — 功能实验室数据（P6）
// 从需求出发实现同一功能：需求拆解 + 知识点连接 + 多语言实现 + 验收测试 + 设计权衡
// 依据《Code Atlas 2.0 产品策划与内容架构执行文档》P6 功能实验室
window.CODE_ATLAS_LAB = {
  "tasks": [
  {
    "id": "lab.python",
    ...
    "phases": [
      {
        "title": "阶段 1：核心数据模型（内存版）",
        "tasks": [
          "定义 Task 类：id、内容、是否完成、优先级、创建时间",
          "实现 TodoList 类：add / list / done / delete 四个方法",
          "先用 Python 交互式 REPL 手动调用验证逻辑"
        ],
        "checkpoints": [
          "能创建任务并打印带编号的任务列表",
          "done/delete 对不存在的 id 有明确处理"
        ]
      },
      {
        "title": "阶段 2：文件持久化",
        "tasks": [
          "选择存储位置（如 ~/.todo.json，可用 pathlib 处理路径）",
          "实现 save() 与 load()：把任务列表序列化为 JSON",
          "程序启动时自动加载、退出时自动保存"
        ],
        "checkpoints": [
          "多次运行程序数据不丢失",
          "JSON 文件损坏时程序不崩溃，给出友好提示"
        ]
      },
      {
        "title": "阶段 3：命令行交互",
        "tasks": [
          "用 argparse 定义子命令：todo add/list/done/delete",
          "支持 --all 显示已完成任务、--priority 过滤",
          "美化输出（编号、状态标记、颜色可选）"
        ],
        "checkpoints": [
          "todo add 买菜 能正确创建",
      ...
      },
      {
        "title": "阶段 4：测试与打磨",
        "tasks": [
          "为 TodoList 核心逻辑编写 pytest 测试（临时目录 fixture）",
          "补充边界：空列表、重复内容、超长内容",
          "编写 README 说明用法"
        ],
        "checkpoints": [
    ...
    "cross_lang": {
      "javascript": "用 Node.js + 文件系统模块实现同等 CLI，JSON 持久化",
      "java": "Scanner 交互 + 集合 + 文件读写，Maven 工程",
      "cpp": "vector + fstream CSV 持久化，CMake 构建",
      "go": "flag 包解析参数 + encoding/json，单二进制分发",
      "rust": "clap 参数 + serde_json + 单元测试"
    }
  },
  {
    "id": "lab.javascript",
    ...
    "techPoints": [
      ...
      "ES Modules 模块化",
      "防抖 / 节流",
      "input/textarea 事件与表单校验"
    ],
    "phases": [
      {
        "title": "阶段 1：静态渲染与新增",
        "tasks": [
          "搭建 index.html 结构：输入区 + 列表区",
          "用模板字符串渲染笔记卡片（标题、内容、时间）",
          "实现「新增笔记」并立即渲染到列表"
        ],
        "checkpoints": [
          "输入内容后点保存，卡片出现在列表顶部",
      ...
      },
      {
        "title": "阶段 2：持久化与编辑删除",
        "tasks": [
          "设计存储格式：notes 数组，含 id/title/body/updatedAt",
          "读取时解析、写入时序列化，处理异常 JSON",
          "实现删除（事件委托）与编辑（点击卡片进入编辑态）"
        ],
        "checkpoints": [
          "刷新页面数据保留",
          "删除单条笔记生效",
      ...
      },
      {
        "title": "阶段 4：模块化重构",
        "tasks": [
          "拆分为 store.js（数据层）/ render.js（视图层）/ app.js（入口）",
          "用 ES Module import/export 连接各模块",
          "处理 localStorage 存储满或损坏的边界"
        ],
        "checkpoints": [
          "type=module 下功能不回归",
          "数据层与视图层可独立修改"
    ...
    "cross_lang": {
      "python": "可用 tkinter/PyQt 或 Flask + HTML 实现同款 SPA",
      "java": "Spring Boot + Thymeleaf 或 JSP 实现服务端渲染版",
      "cpp": "非典型场景；可用 Qt Widgets 实现桌面版",
      "go": "html/template 服务端渲染 + 内存存储",
      "rust": "Tauri + Web 前端 或 actix-web + 静态页"
    }
  },
  {
    "id": "lab.java",
    ...
    "techPoints": [
      "类与封装（Book / Library / BorrowRecord）",
      "集合框架 ArrayList / HashMap",
      "文件 IO（BufferedReader/Writer）与 CSV",
      "异常处理（自定义异常）",
      "JUnit 5 单元测试",
      "Maven 项目结构"
    ],
    "phases": [
      {
        "title": "阶段 1：类设计与内存 CRUD",
        "tasks": [
          "定义 Book（id、书名、作者、ISBN、是否在馆）",
          "定义 Library 类：addBook / findBook / removeBook",
          "控制台菜单：1 添加 2 查询 3 删除 4 退出"
        ],
        "checkpoints": [
          "通过菜单完成增删查",
          "findBook 支持按书名模糊搜索"
      ...
      },
      {
        "title": "阶段 2：文件持久化",
        "tasks": [
          "设计 CSV 存储格式（每行一本书，含各字段）",
          "启动加载 books.csv 到内存，退出写回",
          "处理文件不存在与格式错误"
        ],
        "checkpoints": [
          "重启程序数据保留",
      ...
      },
      {
        "title": "阶段 3：借还业务",
        "tasks": [
          "新增 BorrowRecord 记录借出人、日期",
          "借书：书在馆才能借；还书：校验记录存在",
          "统计在馆/借出数量，列出逾期书目（假设 30 天）"
        ],
        "checkpoints": [
          "重复借同一本书被拒绝",
          "还书后库存恢复且记录更新"
      ...
      },
      {
        "title": "阶段 4：异常处理与测试",
        "tasks": [
          "定义 BookNotFoundException、BookUnavailableException",
          "用 JUnit 为 Library 核心逻辑写测试（含临时文件）",
          "完善菜单输入校验（数字越界、空输入）"
        ],
        "checkpoints": [
          "mvn test 全部通过",
    ...
    "cross_lang": {
      "python": "dataclass + json 存储，逻辑更简洁",
      "javascript": "Node + 对象字面量，弱类型需更谨慎校验",
      "cpp": "struct + vector + CSV，手动管理复制语义",
      "go": "struct + slice + encoding/csv",
      "rust": "struct + serde + 所有权约束更严"
    }
  },
  {
    "id": "lab.cpp",
    ...
    "phases": [
      ...
      },
      {
        "title": "阶段 2：排序与排名",
        "tasks": [
          "按总分从高到低排序（std::sort + lambda）",
          "输出排名表：名次 姓名 总分 平均分",
          "支持按单科成绩切换排序依据"
        ],
        "checkpoints": [
          "相同分数名次并列处理合理",
          "排序不改变原始录入顺序（拷贝而非原地破坏）"
        ]
      },
      {
        "title": "阶段 3：文件持久化",
      ...
      },
      {
        "title": "阶段 4：工程化与调试",
        "tasks": [
          "拆分为 student.h/cpp、stats.h/cpp、main.cpp",
          "用 CMake 构建（add_executable 多源文件）",
          "为统计函数加断言或简单测试；用调试器定位一处边界 bug"
        ],
        "checkpoints": [
    ...
    "cross_lang": {
      "python": "列表 + 字典推导极简实现",
      "javascript": "数组 sort + reduce 声明式实现",
      "java": "Stream + Comparator 更少样板代码",
      "go": "sort.Slice + 结构体切片",
      "rust": "Vec + 迭代器链 + Ord derive"
    }
  },
  {
    "id": "lab.go",
    ...
    "techPoints": [
      ...
      "os/signal 优雅退出",
      "并发安全（互斥或 channel 串行化）",
      "go test -race 竞态检测"
    ],
    "phases": [
      {
        "title": "阶段 1：任务模型与串行执行",
        "tasks": [
          "定义 Task 结构（id、执行耗时、结果字段）",
          "实现串行执行器：依次执行任务并打印结果",
          "模拟任务用 time.Sleep 表示耗时"
        ],
        "checkpoints": [
          "任务按顺序完成，结果正确"
        ]
      ...
      {
        "title": "阶段 2：Worker Pool 并发",
        "tasks": [
          "实现固定数量 worker，从 jobs channel 取任务",
          "结果通过 results channel 回传",
          "用 WaitGroup 等待全部 worker 结束"
        ],
        "checkpoints": [
          "并发执行总耗时明显小于串行",
          "n 个任务全部有结果，无丢失无重复"
      ...
      },
      {
        "title": "阶段 3：超时与取消",
        "tasks": [
          "用 context.WithTimeout 控制整体执行时限",
          "超时后停止派发新任务，已在执行的等待完成",
          "处理 Ctrl+C（os/signal）优雅退出并打印统计"
        ],
        "checkpoints": [
          "超时后程序不悬挂、能正常退出",
          "统计信息（成功/失败/跳过）正确"
    ...
    "cross_lang": {
      "python": "concurrent.futures ThreadPoolExecutor 等价",
      "javascript": "Promise.all 并行（单线程异步）",
      "java": "ExecutorService + Future",
      "cpp": "std::thread + future 或 OpenMP",
      "rust": "tokio::spawn + JoinSet 或 rayon"
    }
  },
  {
    "id": "lab.rust",
    ...
    "techPoints": [
      "所有权与借用（&str vs String）",
      "Result / Option / ? 运算符",
      "自定义错误类型（thiserror 或手写 Display）",
      "迭代器链式编程",
      "std::fs 目录递归（WalkDir 或手写栈）",
      "命令行参数解析（clap 或手写）",
      "cargo test 集成测试"
    ],
    "phases": [
      {
        "title": "阶段 1：单文件搜索",
        "tasks": [
          "读取单个文件（std::fs::read_to_string）",
          "按行迭代，输出包含关键词的行号与内容",
          "处理文件不存在与编码异常"
        ],
        "checkpoints": [
          "cargo run -- pattern file.txt 正确输出匹配行",
          "错误场景返回非零退出码"
        ]
      },
      {
        "title": "阶段 2：递归目录遍历",
        "tasks": [
          "实现目录递归（手写 Vec<PathBuf> 栈，或引入 walkdir 依赖）",
          "跳过隐藏文件与二进制文件",
          "输出格式：文件名:行号:内容"
        ],
        "checkpoints": [
          "嵌套目录全部覆盖",
          "无匹配时输出提示且退出码为 1（类 grep 约定）"
        ]
      },
      {
        "title": "阶段 3：选项与结果整理",
        "tasks": [
          "支持 -i 忽略大小写、-n 仅行号、--count 统计",
          "结果按文件名与行号排序",
          "用迭代器链重构匹配逻辑"
        ],
        "checkpoints": [
      ...
      },
      {
        "title": "阶段 4：错误处理与测试",
        "tasks": [
          "定义 SearchError 统一包装 IO/解析错误",
          "为匹配函数写单元测试（含空文件、大小写）",
          "为 CLI 行为写集成测试（tests/ 目录）"
        ],
        "checkpoints": [
    ...
    "cross_lang": {
      "python": "os.walk + re 正则，注意与 Rust 借用模型差异",
      "javascript": "fs.readdir 递归 + RegExp",
      "java": "Files.walk + Pattern，受检异常处理",
      "cpp": "std::filesystem 递归 + 正则",
      "go": "filepath.WalkDir + regexp"
    }
  }
]
};
